...
bool IfCommand::checkIsDouble(string inputString, double &result) {
    char* end;
...
void IfCommand::cal(string leftString, string rightString){
    double result;
    double value;
    string inputSetVars;
...
bool IfCommand::isCondCorrect(vector<string>::iterator &it){
    Interpreter* interpreter = new Interpreter();
...
void IfCommand::execute(vector<string>::iterator &it){
    bool isCorrect = this->isCondCorrect(it);
        ...
        for (unsigned int i = 0; i < this->scope.size(); i++){
            ...
            if (this->commandsMap.count(firstToken)){
                ...
                if (this->symbolTable->isInVarSymbols(lastToken)){
                    this->symbolTable->updateSymbol(firstToken,
                                                    this->symbolTable->getSymbolValue(lastToken));
                } else {
                    // the last token is an expression or a number
                ...
                if (this->symbolTable->getSymbolOper(firstToken).compare("->")== 0){
                    //this->updateSimVar->needUpdate = true;
                    auto it = this->scope.at(i).begin();
